...
print(list(dataframe_1.select('State').
           toPandas()['State']))

# display student NAME  column in
# the list format using toPandas
print(list(dataframe_1.select('Cases').
           toPandas()['Cases']))

# display subject1  column in
# the list format using toPandas
print(list(dataframe_1.select('Recovered').
           toPandas()['Recovered']))

# display subject2  column
# in the list format using toPandas
print(list(dataframe_1.select('Deaths').
           toPandas()['Deaths']))

print(dataframe_1.select('*').
      rdd.map(lambda x: x['State']).collect())

print(list(dataframe_1.select(dataframe_1['*'])))
print(list(dataframe_1.alias("a").select("a.*").toPandas()))
print(dataframe_1.alias("a").select("a.*").toPandas())
print(dataframe_1.alias("a").select("a.*").toPandas()['State'])