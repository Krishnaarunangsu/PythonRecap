...
class MapValuesTransformation:
    ...
    def create_rdd(self):
        ...
        print(sorted(rdd_key_value.groupByKey().mapValues(len).collect()))
        print(sorted(rdd_key_value.groupByKey().mapValues(list).collect()))


if __name__=="__main__":
    tuple_list:list=[("a", 1), ("b", 1), ("a", 1)]
    map_values_transformation=MapValuesTransformation(tuple_list)
    map_values_transformation.create_rdd()
